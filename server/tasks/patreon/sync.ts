...
import { PatreonClient } from '../../utils/patreon-client';
...
export default defineTask({
  ...
  async run({ payload, context }) {
    ...
    try {
      ...
      const targetCollectionId = payload?.collection as string | undefined;
      
      // Filter collections if specific one requested
      const collectionsToSync = targetCollectionId
        ? config.patreon.collections.filter(c => c.id === targetCollectionId)
        : config.patreon.collections;
      
      if (collectionsToSync.length === 0) {
        throw new Error(`Collection ${targetCollectionId} not found in config.json`);
      }
